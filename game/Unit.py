...
class Infantry(Unit):
    def __init__(self, owner, position, image_path, health = DEFAULT_HEALTH, moves = DEFAULT_MOVES):
    ...
    def move_to(self, new_hex):
        ...
        if pos in self.current_hex.neighbor_list:
            if isinstance(new_hex.unit, Unit) and self.owner != new_hex.owner:
                ...
                self.moves -= 1
                cp_unit = Infantry(self.owner, new_hex, self.img_path, self.health, self.moves)
